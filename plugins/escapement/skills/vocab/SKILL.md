...
1. **Directly** — invoke `$vocab` (Codex) or `/skill:vocab` (Pi) any time, for standalone
...
subagent), organizes findings by seed-term slot, and emits a
...
message. Anything that lives only in a message is lost when the agent shuts
down (the exact bug these runs hit — findings had to be rebuilt forensically from
message payloads). Three instances of the one principle: findings → file;
