...
tools: read, find, grep, bash, mcp:escapement__serena/find_symbol, mcp:escapement__serena/get_symbols_overview, mcp:escapement__serena/find_referencing_symbols, mcp:escapement__serena/search_for_pattern, mcp:escapement__serena/find_file, mcp:escapement__serena/list_memories, mcp:escapement__serena/read_memory
...
the dispatcher typed into the `task` field. That prompt is SUSPECT.
...
2. **Read it yourself.** Prefer Serena (`find_symbol`, `get_symbols_overview`,
   ...
   for code navigation; fall back to `read`, `grep` for non-code files,
