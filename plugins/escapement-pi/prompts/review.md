...
Dispatch all agents simultaneously. Each agent gets the same target files/PR but reviews from a different angle. Launch all three in ONE `subagent` call: pass the three items below to `runs.all([...])` inside its `workflowScript` and return every child's output.
...
```
{
  ...
  task: `You are an adversarial code reviewer. Your job is to BREAK this code.
...
```
{
  ...
  task: `You are a test quality reviewer. Your job is to find gaps between what the tests claim to verify and what they actually verify.
...
```
{
  key: "code-simplifier",
  agent: "delegate",
  task: `You are a code simplicity reviewer. Your job is to find unnecessary complexity in the changes.
...
After the `subagent` call returns every child's output, synthesize their findings into a unified review:
