...
1. **Explore project context** — Read AGENTS.md (and CLAUDE.md, if present), relevant code, and existing docs to understand the landscape.
...
> load-bearing — load the **`vocab`** skill (`$vocab` on Codex, `/skill:vocab` on Pi) to recover the field's real
...
- **Grounded in project context.** Read AGENTS.md (and CLAUDE.md, if present), existing design docs, PRDs, and any relevant project files before writing the probe. Generic risks ("this could be complex") are not acceptable.
...
`openspec/changes/{name}/problem-framing.md`. Then immediately load the `discovery` skill from your skills catalog (read its SKILL.md) and follow it for `<feature-name>` in this same turn. The user-facing shortcut is
`$discovery <feature-name>` on Codex and `/skill:discovery <feature-name>` on Pi.

Do NOT re-pass the framing through the skill arguments — it lives in `problem-framing.md`.
