...
slots:
  deep_research_guard:
    claude: |-
      ...
      > — there isn't one.
    codex: |-
      > **Implementation guard — do not "amend" deep-research.** Where a
      > `deep-research` skill exists it is a separate skill (Claude Code's is compiled
      > in, with **no editable source**); vocab-first is a thin wrapper that **calls**
      > it for Phase 2 — it is NOT a phase prepended inside deep-research, NOT a discovery
      > mode (discovery's Input Gate requires confirmed framing; this runs pre-framing),
      > and NOT a `dispatching-parallel-agents` use (that skill excludes sequential
      > dependencies; this is one). Don't go looking for deep-research's source to edit.
  phase_ownership:
    ...
    codex: |-
      This skill owns **Phase 1 only** (terminology recovery) and **delegates Phase 2
      to a `deep-research` skill** when the host has one. It does not re-implement
      fan-out / fetch / verify / synthesize beside an existing research route — that
      would be a second route to the same output (the
      [[feedback_workflow_simplification]] "two routes = retire one" collision).
      `deep-research` has no terminology phase; this is the missing **outward
      front-end** that feeds it. On Codex or Pi without an installed `deep-research`
      skill, each Phase-2 lane is itself the research route: run it as a research
      subagent (Pi: the builtin `researcher` agent through the `subagent` tool; Codex:
      a `spawn_agent` child with web search) that does the fan-out / fetch / verify /
      synthesize for that one lane.
  direct_invocation:
    claude: "invoke `/vocab` (or via the Skill tool) any time"
    codex: "invoke `$vocab` (Codex) or `/skill:vocab` (Pi) any time"
  phase2_delegate:
    claude: |-
      the
      `deep-research` skill, organizes
    codex: |-
      the
      `deep-research` skill (or, with none installed, runs it itself as a research
      subagent), organizes
  wire_message:
    claude: "SendMessage"
    codex: "message"
targets:
  claude: claude/skills/vocab/SKILL.md
  codex: .agents/skills/vocab/SKILL.md
frontmatter:
  claude:
    name: "vocab"
...
Each lane searches **using the vocabulary**, hands the fan-out to {{slot:phase2_delegate}} findings by seed-term slot, and emits a
...
message. Anything that lives only in a {{slot:wire_message}} is lost when the agent shuts
down (the exact bug these runs hit — findings had to be rebuilt forensically from
{{slot:wire_message}} payloads). Three instances of the one principle: findings → file;
