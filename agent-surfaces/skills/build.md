...
slots:
  ...
  auto_detect_agent:
    ...
    codex: "The agent"
  usage:
    claude: "/build {description of what to build}"
    codex: |-
      $build {description of what to build}        # Codex
      /skill:build {description of what to build}  # Pi
  beads_setup:
    claude: |-
      ...
      ```
    codex: |-
      Initialize beads (and, on Codex, its Codex integration) automatically:
      ```bash
      bd init
      bd setup codex   # Codex only; bd has no Pi recipe, so on Pi stop after bd init
      ```
  self_reference:
    claude: "`/build`"
    codex: "this skill"
targets:
  claude: claude/skills/build/SKILL.md
...
Optional explicit trigger for the mol-feature pipeline. {{slot:auto_detect_agent}} should also
...
set up beads manually — {{slot:self_reference}} handles it.
