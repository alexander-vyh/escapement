---
op: dispatching-parallel-agents
slots:
  completeness_critic_example:
    claude: |-
      ```
  ...
  independent_test_example:
    ...
    codex: |-
      ...
      ```
  shutdown_verify:
    claude: "5. **Shut down the team** — SendMessage shutdown_request to each agent"
    codex: "5. **Shut down the team** — stop any agent still running (Codex `interrupt_agent`; Pi `subagent({ action: \"stop\", id })`)"
  naming_mistakes:
    claude: |-
  ...
  named_structure:
    claude: "1. **Named** — `name` parameter on every Agent call"
    codex: "1. **Named** — a `task_name` (Codex) or `key` (Pi) on every spawn"
  roundtable_example:
    claude: |-
      ```
      Agent(
    ...
    codex: |-
      ...
      ```
  shutdown_integrate:
    claude: "- Shut down the team when done"
    codex: "- Stop any agent still running when done (Codex `interrupt_agent`; Pi `subagent({ action: \"stop\", id })` for a live async run)"
  dispatch_example:
    claude: |-
      ```
      Agent(
  ...
  name_mandate:
    claude: "**MANDATORY: Every agent MUST have a `name`.** The hook will block anonymous agents."
    codex: "**MANDATORY: Every agent MUST have a name** — Codex `task_name`, Pi `key`. An anonymous agent cannot be messaged, followed up, or told apart in the results."
  intro:
    claude: "This skill provides named-agent dispatch for parallel work. All agents MUST have a `name` so they can communicate via SendMessage."
    codex: |-
  ...
  peer_channel_code:
    claude: "`SendMessage`"
    codex: "messages (relayed through you on Pi)"
  send_instructions:
    claude: "SendMessage"
    codex: "send"
  peer_channel:
  ...
  forage_tools:
    claude: "Read/Grep"
    codex: "its read and search tools"
targets:
  claude: claude/skills/dispatching-parallel-agents/SKILL.md
  codex: .agents/skills/dispatching-parallel-agents/SKILL.md
frontmatter:
  claude:
...
**Core principle:** Dispatch one **named** agent per independent problem domain. They work concurrently and communicate via {{slot:peer_channel_code}}.
...
{{slot:shutdown_integrate}}

## Roundtable Pattern

A "roundtable" is a specific use of team agents where expert personas **argue and critique each other's positions** via {{slot:peer_channel}}.

{{slot:roundtable_example}}

## Agent Prompt Structure

Good agent prompts are:
{{slot:named_structure}}
2. **Focused** — One clear problem domain
3. **Self-contained** — All context needed to understand the problem
4. **Peer-aware** — Lists other agents' names so they can coordinate via {{slot:peer_channel}}
...
with {{slot:forage_tools}}. Pre-quoted code is pre-interpreted code — interpretation is
...
{{slot:naming_mistakes}}

**❌ Simulated roundtable:** Writing persona dialogue in your own output
**✅ Real roundtable:** Named agents that independently analyze and argue via {{slot:peer_channel}}
...
| "roundtable" | Named agents arguing via {{slot:peer_channel}} |
| "panel of experts" | Named agents with different persona prompts |
| "have them talk to each other" | Named agents using {{slot:peer_channel}} |

**"Roundtable" NEVER means writing simulated dialogue in your output.** It ALWAYS means real named agents that independently analyze and communicate via {{slot:peer_channel}}.
...
dispatch a follow-up agent or {{slot:send_instructions}} instructions to fix them. If the
...
{{slot:completeness_critic_example}}
