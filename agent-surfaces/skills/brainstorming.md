...
slots:
  project_instructions:
    claude: "CLAUDE.md"
    codex: "AGENTS.md (and CLAUDE.md, if present)"
  vocab_invocation:
    claude: "(`/vocab`)"
    codex: "(`$vocab` on Codex, `/skill:vocab` on Pi)"
  discovery_invocation:
    claude: |-
      ...
      ```
    codex: |-
      Then immediately load the `discovery` skill from your skills catalog (read its SKILL.md) and follow it for `<feature-name>` in this same turn. The user-facing shortcut is
      `$discovery <feature-name>` on Codex and `/skill:discovery <feature-name>` on Pi.
  framing_args:
    claude: "`args`"
    codex: "the skill arguments"
  work_breakdown_invocation:
    claude: |-
      Then immediately invoke the Skill tool:
      ```
      ...
      ```
    codex: |-
      Then immediately load the `work-breakdown` skill from your skills catalog (read its SKILL.md) and follow it in this same turn. The user-facing shortcut is
      `$work-breakdown` on Codex and `/skill:work-breakdown` on Pi.
targets:
...
1. **Explore project context** — Read {{slot:project_instructions}}, relevant code, and existing docs to understand the landscape.
...
> load-bearing — load the **`vocab`** skill {{slot:vocab_invocation}} to recover the field's real
...
- **Grounded in project context.** Read {{slot:project_instructions}}, existing design docs, PRDs, and any relevant project files before writing the probe. Generic risks ("this could be complex") are not acceptable.
...
`openspec/changes/{name}/problem-framing.md`. {{slot:discovery_invocation}}

Do NOT re-pass the framing through {{slot:framing_args}} — it lives in `problem-framing.md`.
...
{{slot:work_breakdown_invocation}}
